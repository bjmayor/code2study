...
** 线程属性
   ...
   * 并发度
   ...
   并发度控制着用户级线程可以映射的内核线程或进程的数目。如果操作系统的实现在内核级线程和用户级的线程之间保持一对一的映射，那么改变并发度并不会有效果。
...
*** 互斥量属性
    ...
    互斥量健壮属性与在多个进程间共享的互斥亘有关。这意味着，当持有互斥量的进程终止时，
...
** 线程特定数据
   ...
   引入线程私有数据的原因：
   1. 有时候需要维护基于线程的数据。防止某个线程的数据与其他线程的数据相混淆。
