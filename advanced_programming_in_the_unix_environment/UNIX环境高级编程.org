...
** 登录
口令文件 /etc/passwd 条目说明：
登录名:加密口令:数值用户ID:数值组ID:注释字段:起始目录:shell程序
把shell程序 设置为/sbin/nologin 可以禁止用户登录。一般给mysql等程序设定的用户都使用/sbin/nologin
** 文件与目录
   UNIX文件系统的大多数实现并不在目录项中存放属性。有inode就够。

   程序的正确性是由硬件->内核->系统调用->标准库函数->第三方库->程序员 逐步保证的。
   
   每个进程都有一个工作目录。 相对路径和工作目录可以构造为绝对路径。
   用户登录时，工作目录设为超始目录，可以通过chdir函数更改工作目录。`cd`命令也是更改工作目录。`pwd`可以查看当时工作目录。
** 输入输出
*** 文件描述符
内核用它标识一个特定进程正在访问的文件。
*** 标准输入，标准输出，标准出错
...
*** 不带缓冲的I/O
函数open、read、close、lseek提供了不带缓冲的I/O。这些函数都使用文件描述符。
*** 标准I/O
标准I/O函数提供一种对不带缓冲I/O函数的带缓冲的接口。这样，你就不用担心如何选取最佳的缓冲区大小。而且简化了对输入行的处理。
** 程序与进程
...
** 用户标识
   对于权限，使用数值用户ID和数值组ID是历史上形成的。1是数字比字符串的存储空间小,每个文件都会存放用户ID和组ID;2是查验权限期间，比较整数更快。
...
** 时间值
   ...
   1. 日历时间。自1970年1月1日00:00:00以来UTC所经过的秒数。 用time_t保存。
   2. 进程时间。也被称为CPU时间，用来度量进程使用的中央处理器资源。用clock_t保存。进程时间以时钟滴答计算，历史上曾经取每秒钟为50、60、或100个。具体的值可以通过sysconf获取。
      1. 时钟时间
      2. 用户CPU时间
      3. 系统CPU时间。
** 系统调用和库函数         
UNIX所使用的技术是为每个系统调用在标准C库中设置一个同名的函数。用户进程用标准C调用序列来调用这些函数，然后，函数又用系统调用所要求的技术调用相应的内核服务(系统调用)。
例如函数可将一个或多个C参数送入通用寄存器，然后执行某个产生软中断进入内核的机器指令。从应用角度考虑，可将系统调用视作C函数。
从实现者角度看，系统调用和库函数之间有重大的区别。必要时，我们可以替换库函数，而通常却不能替换系统调用。
...
编译时限制可以头文件中定义，程序在编译时可以包含这些头文件。但是，运行时限制则要求进程调用一个函数以获得此种限制值。

另外，某些限制在一个给定的实现中可能是固定的(因此可以静态地在一个头文件中定义),而在另一个实现上则可能是变化的。
为了解决这类问题，提供了以下三种限制：
1. 编译时限制(头文件)。
...
** dup和dup2函数
   ...
   由dup返回的新文件描述符一定是当前可用文件描述符中的最小数值。用dup2则可以用fieldes2参数指定新描述符的数值。如果filedes2已经打开，则先将其关闭。如若filedes等于
...
*** 屏障
    屏障(barrier)是用户协调多个线程并行工作的同步机制。屏障允许每个线程等待，直到所有的合作线程都
    到达某一点，然后从该点继续执行。
...
** FIFO
   <2016-06-22 Wed>
   FIFO有时被称为命名管道。未命名的管道只能在两个相关的进程之间使用，而且这两个相关的进程还要有一个共同的创建了
   它们的祖先进程。但是，通过FIFO，不相关的进程也能交换数据。
   
   创建FIFO类似创建文件。硬实，FIFO的路径名存在于文件系统中。
   #include <sys/stat.h>
   int mkfifo(const char *path, mode_t mode);
   int mkfifoat(int fd, const char *path, mode_t mode);
   
   mkfifoat函数和mkfifo函数类似，但是mkfifoat函数可以被用来在fd文件描述符表示的目录相关的位置创建一个FIFO。
   像其他*at函数一样，这里有3种情形：
   1. 如果path参数指定的是绝对路径名,fd会忽略。
   2. 如果path参数指定的是相对路径名，则fd参数是一个打开目录的有效文件描述符，路径名和目录有关。
   3. 如果path参数指定的是相对路径名，并且fd参数有一个特殊值AT_FDCWD,则路径名以当前目录开始，
      mkfifoat和mkfifo类似。
     
      
   当open一个FIFO时，非阻塞标志(O_NONBLOCK)会产生下列影响。
   1. 在一般情况下(没有指定O_NONBLOCK),只读open要阻塞到某个进程为写而打开这个FIFO为止。类似地，只写open要阻塞到
      某个其他里程为读而打开它为止。(需要阻塞到有读有写)
   2. 如果指定了O_NONBLOCK,则只读open立即返回。但是，如果没有进程为读而打开一个FIFO,
      那么只写open将返回-1,并将errno设置成ENXIO。
      
      
   类似于管道，若write一个尚无进程为读而打开的FIFO,则产生信号SIGPIPE。若某个FIFO的最后一个写进程关闭了该FIFO,则将为该
   FIFO的读进程产生一个文件结束标志。
   ...
   2. 客户进程-服务器进程应用程序中，FIFO用作汇聚点，在客户进程和服务器进程二者之前传递数据。
** XSI IPC
   有3种称为XSI IPC的IPC: 消息队列、信号量以及共享存储器。
*** 标志符和键
    每个内核中的IPC结构(消息队列、信号量或共享存储器)都用一个非负整数的标识符加以引用。
    当一个IPC结构被创建，然后又被删除时，与这种结构相关的标识符连续加1，直至达到一个整型数的最大正值，然后又回转到0.
    
    标识符是IPC对象的内部名。为使多个合作进程能够在同一IPC对象上汇聚，需要提供一个外部命名方案。
    为此，每个IPC对象都与一个键(key)相关联，将这个键作为该对象的外部名。
    
    无论何时创建IPC结构(通过调用msgget、semget或shmget创建),都应指定一个键。
    这个键的数据类型是基本系统数据类型key_t,通常在头文件<sys/types.h>中被定义为长整型。这个键由内核变换成标识符。

    有多种方法使客户进程和服务进程在同一IPC结构上汇聚。
    1. 服务器进程指定键IPC_PRIVATE创建了个新IPC结构，然后将返回的标识符存放在某处(如文件)以
       便客户进程取用。
    2. 可以在一个公用头文件中定义一个客户和服务器进程都认可的键。然后服务器进程指定此键创建
       一个新的IPC结构。问题是可能该键已与IPC结构相结合，服务器进程需要处理错误返回，删除已存在的IPC结构，然后试着
       再创建它。
    3. 客户进程和服务器进程认同一个路径名和项目ID(项目ID是0~255之间的字符值)。
       接着调用函数ftok将这两个值转成一个键。然后使用方法2.
       #include <sys/ipc.h>
       key_t ftok(const char *path, int id);
       path参数必须引用一个现有的文件。
       
       3个get函数(msgget、semget和shmget)都有两个类似的参数：一个key和一个整形flag。
       在创建新的IPC结构时，如果key是IPC_PRIVATE或者和当前某种类型的IPC结构无关，则需要指明flag的IPC_CREAT标志位。
*** 权限结构
    ...
    struct ipc_perm {
        ...
        gid_t cgid; /* creator's feective group id */
        mode_t mode;/* access modes */
    };
*** 结构限制
    所有3种形式的XSI IPC都有内置限制。大多数限制可以通过重新配置内核来改变。
    在Linux中，可以运行ipcs -l 显示限制。
    在FreeBSD中，等效的命令是ipcs -T。
*** 优点和缺点
    缺点：
    1. IPC结构是在系统范围内起作用的，没有引用计数。
       进程终止时，并不会删除其创建的消息队列和内容。
       删除时机是：由某个进程调用msgrcv或msgctl读消息或删除消息队列；
       或某个进程执行ipcrm命令删除消息队列；
       或正在自举的系统删除消息队列。
    2. 这些IPC结构在文件系统中没有名字。为了管理，增加了msgget、semop、shmat,ipcs,ipcrm等命令。
    3. 由于这些形式的IPC不使用文件描述符，所以不能对它们使用多路转接I/O函数。
    ...
    | FIFO               | 否      | 是      | 是      | 是    | 否                |
    "无连接"指无需调用打开函数就能发送消息。
    "流控制"的意思是：如果系统资源(缓冲区)短缺，或者如果接收进程不能再接收更多消息，
    则发送进程就要休眠。当流控制条件消失时，发送进程自动唤醒。
** 消息队列
   消息队列是消息的链接表，存储在内核中，由消息队列标识符标识。
   msgget用户创建一个新队列或打开一个现有队列。
   msgsnd用于将消息添加到队列尾端。
   msgrcv用于从队列中取消息。
   
   我们并不一定要以行进先出次序取消息，也可以按消息的类型字段取消息。
   ...
   限制
   | 说明                                                 | FreeBSD8.0 | Linux3.2.0 | Mac OS X 10.6.8 | Solaris 10 |
   |------------------------------------------------------+------------+------------+-----------------+------------|
   | 可发送的最长消息的字节数                             |      16384 |       8192 | 不支持          |       2048 |
   | 一个特定队列的最大字节数(即队列中所有消息的长度之和) |       2048 |      16384 | 不支持          |       4096 |
   ...
   IPC_SET 从字段msg_perm.uid、msg_perm.gid、msg_perm.mode和msg_qbytes从buf指向的结构
           复制到这个队列相关的msqid_ds结构中。
   IPC_RMID 从系统中删除该消息队列以及扔在该队列中的所有数据。这种删除立即生效。
   ...
   参数flag的值可以指定为IPC_NOWAIT,若消息队列已满，则指定的IPC_NOWAIT使得msgsnd立即出错返回EAGAIN。

   msgrcv从队列中取消息。
   #include <sys/msg.h>
   ssize_t msgrcv(int msqid, void *ptr, size_t nbytes, long type, int falg);
   若返回的消息长度大于ntyes,而且在flag中设置了MSG_NOERROR位，则该消息会被截断(消息被截去的部分丢弃)。如果没有设置这一标志，
   而消息又太长，则出错返回E2BIG(消息仍留在队列中)。
   ...
   ** 在新的应用程序中不应当再使用消息列表(无效率优势，又有各种缺陷) **
** 信号量
   ...
   3. 否则，若此信号量的值为0，则进程进入休眠状态，直至信号量值大于0.进程被唤醒后，它返回到步骤1.
      
   当进程不再使用由一个信号量控制的共享资源时，该信号量值增1。如果有进程正在休眠等待此信号量，则唤醒它们。
   
   常用的信号量形式被称为二元信号量。它控制单个资源，其初始值为1。但是，一般而言，信号量的初值可以是任意一个正值，
   该值表明有多少个共享资源单位可供共享应用。

   实际要复杂很多，体现在：
   1. 信号量并非是单个非负值，而必需定义为含有一个或多个信号量值的集合。当创建信号量时，要指定集合中信号量值的数量。
   2. 信号量的创建(semget)是独立于它的初始化(semctl)的。这是一个致命的缺点，因为不能原子地创建一个信号量集合，
      并且对该集合中的各个信号量值赋初值。
   ...
   信号量的系统限制
   | 说明               | FreeBSD8.0 | Linux 3.2.0 | Mac OS X 10.6.8 | Solaris 10 |
   |--------------------+------------+-------------+-----------------+------------|
   | 任一信号量的最大值 |            |             |                 |            |
   | 任一信号量的最大退出时的调整值 |            |             |                 |            |
   | 系统中信号量集的最大数量       |            |             |                 |            |
   ...
   semop函数具有原子操作，它或者执行数组中的所有操作，或者一个也不做。
   
   exit时的信号量调整
   无论何时只要为信号量操作指定了SEM_UNDO标志，然后分配资源(sem_op值小于0),那么内核就会记住对于该特定信号量，
   分配给调用进程多少资源(sem_op的绝对值)。当该进程终止时，不论自愿或者不自愿，内核都将检验该进程是否还有尚未处理的
   信号量调整值，如果有，则按调整值对相应信号量进行处理。

   如果在多个进程间共享一个资源，可使用信号量、记录锁和互斥量三种技术中的一种来协调访问。
   效率上， 共享存储中的互斥量>建议性记录锁>带undo的信号量。
   实际中我们更倾向于使用记录锁：
   1. 在多个进程间共享的内存中使用互斥量来恢复一个终止的进程更难。
   2. 进程共享的互斥量属性还没有得到普遍支持。
** 共享存储
     共享存储允许两个或更多进程共享一给定的存储区。
     因为数据不需要在客户进程和服务器进程之前复制，所以这是最快的一种IPC。
     使用共享存储时要掌握的唯一窍门是多个进程之间对一给定存储区的同步访问。
     通常，信号量被用来实现对共享存储访问的同步。(记录锁也可用于这种场所)
     
     内核为每个共享存储段设置了一个shmid_ds结构。
     struct shmid_ds {
         struct ipc_perm shm_perm;/* */
         size_t shm_segsz;/* size of segment int bytes */
...
* 第十六章 网络IPC:套接字
** 引言
   套接字的目标之一: 同样的接口既可以用于计算机之间通信又可以用于计算机内通信。
** 套接字描述符
套接字是通信端点的抽象。与应用程序要使用文件描述符访问文件一样，访问套接字也需要用套接字描述符。套接字描述符在UNIX系统是用文件描述符实现的。
创建一个套接字，可以调用socket函数。

#include <sys/socket.h>

int socket(int domain, int type, int protocal);//返回值: 若成功则返回文件描述符, 若出错则返回-1

domain: 确定通信的特性，包括地址格式，都以AF_开头。
| 域        | 描述   |
| AF_INET   | Ipv4   |
| AF_INET6  | IPv6   |
| AF_UNIX   | UNIX域 |
| AF_UNSPEC | 未指定 |
type: 套接字的类型，进一步确定通信特性。
| 类型           | 描述                                   |
...
protocal: 通常为0，表示按给定的域和套接字类型选择默认协议。(AF_INET/SOCK_DGRAM 默认为UDP, AF_INET/SOCK_STREAM默认为TCP)

数据报(SOCK_DGRAM)近似于给某人邮寄信件。
使用面向连接的协议通信就像与对方打电话。
...
int shutdown(int sockdf, int how);//0-成功, -1 出错
#+END_SRC
how: SHUT_RD 关闭读端。 SHUT_WR 关闭写端。 SHUT_RDWR 关闭读和写。

** 寻址
   进程的标识有两个部分：计算机的网络地址可以帮助标识网络上想与之通信的计算机，而服务可以帮助标识计算机上特定的进程。
*** 字节序
    字节序是一个处理器架构特性，用于指示像整数这样的大数据类型的内部字节顺序。
    譬如 0x04030201 用字符指针cp表示话。
    cp[0]  是低地址, cp[3] 是高地址。在大端字节处理器上，高地址对应于数字最低有效字节。cp[3]包含1。 阅读自然，一一对应。TCP/IP协议栈也是采用的大端字节。
    在小端字节处理器上，低地址对应于数字最低有效字节。cp[0]包含1.

    注意: 不管字节如何排序，数字最高位总是在左边, 最低位总是在右边。
    
    对于TCP/IP，地址用网络字节序表示，所以应用程序有时需要在处理器的字节序与网络字节序之间的转换。
    对于TCP/IP, 提供了四个通用函数以实施在处理器字节序和网络字节序之间的转换。

#+BEGIN_SRC C 
#include <arpa/inet.h>
unint32_t htonl(uint32_t hostint32); //返回值，以网络字节序表示的32位整型数
...
记忆: h 指 主机host, n 表示网络network, l表示长long, s表示短short
...
BSD网络软件中包含了函数inet_addr和inet_ntoa, 用于在二进制地址格式与点分十进制字符串表示(a.b.c.d)之间相互转换。这些函数仅用于IPV4地址，但功能相似的两个新函数inet_ntop和inet_pton支持
IPv4和IPv6地址。

#+BEGIN_SRC C
#include <arpa/inet.h>
//返回值，成功则返回地址字符串指针，出错则返回NULL
const char *inet_ntop(int domain, const void *restrict addr, char *restrict str, socklen_t size);

//返回值，成功返回1， 格式无效返回0， 若出错则返回-1
int inet_pton(int domain, const char *restrict str, void *restrict addr);
#+END_SRC

*** 地址查询
通过以下API可以遍历查询/etc/hosts文件信息

#+BEGIN_SRC C
#include <netdb.h>

struct hostent *gethostent(void);//若成功则返回指针，出错则返回NULL
...
struct hostent{
    ...
    int h_length; /*地址列表长度*/
    char **h_addr_list;/*地址列表, 网络字节顺序, 需要转换成struct in_addr指针，如想打印出来，还要调用inet_ntoa*/
}
#+END_SRC

gethostbyname和gethostbyaddr已过时。 下面相似的接口可以替代, 遍历的是/etc/networks和/etc/resolv.conf文件
...
struct netent{
    ...
    char **n_aliases;
    int n_addrtype;
    uint32_t n_net;/*网络号，网络字节序*/
}
#+END_SRC

下面一组接口用于遍历/etc/protocols
...
struct protoent *getprotoent(void);/*以上所有函数的返回值，若成功返回指针，若出错返回NULL*/
...
需要提供主机名字、服务名字，或者两者都提供。主机名字可以是一个节点名或点分十进制记法表示的主机地址。
...
如果 *getaddrinfo* 失败，不能使用perror或strerror来生成错误消息。替代地，调用 *gai_strerror* 将返回的错误码转换成错误消息。
...
#+BEGIN_SRC C
#include <sys/socket.h>
int bind(int sockfd, const struct sockaddr *addr, socklen_t len);//成功则返回0, 若出错则返回-1
#+END_SRC
...
这意味着可以收到这个系统所安装的所有网卡的数据包。

可以通过 *getsockname* 函数获取指定sock所绑定的地址。
...
*** 建立连接
对于SOCK_STREAM和SOCK_SEQPACKET, 需要先和对方建立连接。

#+BEGIN_SRC C
#include <sys/socket.h>

int connect(int sockfd, const struct sockaddr *addr, socklen_t len);//*若成功则返回0， 出错返回-1
#+END_SRC

在connect中指定的地址是服务器地址。如果sockfd此时还没有绑定一个本地地址，则会绑定一个默认地址。

ps: SOCK_DGRAM 也可以调用connect

服务器通过调用listen来宣告可以接受连接请求。
...
#+END_SRC
参数backlog用于提供一个指示，用于表示该进程能接受入队的最大连接请求数据。 其实际值由系统决定，但上限由<sys/socket.h>中SOMAXCONN指定。

一旦队列满，系统会拒绝多余连接请求。

服务调用了listen后，就可以通过 *accept* 来获得连接请求并建立连接。
...
accept 返回的文件描述符就是套接字描述符，该描述符连接到客户端。
这个新的套接字描述符和原始套接字具有相同的套接字类型和地址族。传给accept的原始套接字没有关联到这个连接，而是继续保持可用状态并接受其他连接请求。

如果没有连接请求等待处理, *accept* 会阻塞直到一个请求到来。如果sockfd处于非阻塞模式, accept会返回-1, 并设置errno为EAGAIN或EWOULDBLOCK.
服务器也可使用poll或select来等待一个请求的到来。
