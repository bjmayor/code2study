...
** exec函数
   ...
   * tms_utime、tms_stime、tms_cutime和tims_cstime值。
   ...
   对打开文件的处理和每个描述符的执行时关闭(close-on-exec)标志值有关（FD_CLOEXEC).若此值设置，则在执行exec时关闭描述符，否则保留为打开。系统默认设置为在执行exec后仍保持这种描述符打开。
...
** system函数
   ...
   如果cmdstring是一个空指针，则仅当命令处理程序可用时，system返回非0值，用此特殊判断操作系统是否支持system函数。
