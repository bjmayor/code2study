...
** 引言
   ...
   同步机制，用以防止多个线程查看到一不致的共享资源。
** 线程概念
   使用多线程的好处：
   1. 通过为每种事件类型的处理分配单独的线程，能够简化处理异步事件的代码。每个线程只在进行事件处理进可以采用同步编程模式，同步编程模式要比异步编程模式简单的多。
   ...
   进程的所有信息对该进程的所有线程都是共享的，包括可执行的程序文本、程序的全局内存和堆内存、线以及文件描述符。
...
与它们的注册时相反。
   ...
   当线程执行以下动作时，清理函数rtn是由pthread_cleanup_push函数调度的，调用时
只有一个参数arg:
   * 调用pthread_exit时;
   * 响应取消请求时;
...
得当。
    ...
    互斥变量用pthread_mutex_t 数据类型表示，在使用之前，必须先初孡经，可以设置为常量PTHREAD_MUTEX_INITIALIZER(只对静态分配的互斥量),
